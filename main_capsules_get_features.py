...
test_batch_size = 1
test_split_batch_size = 64
model_param_adr = r'E:\saved_model\capsule_features_fullface_epoch_15_param_celebDF_132_213.pkl' # None if new training

transf = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    # Resnet and VGG19 expects to have data normalized this way (because pretrained)0
])
...
for i in range(len(data_test)):
    ...
    for j in range(len(data_total)//test_split_batch_size):
        ...
        with torch.no_grad():
            try:
                outputs_gpu, features = model(data)
            ...
            features_total.append(features.to('cpu').detach())


    outputs_total = torch.cat(outputs_total).flatten().mean()
    features_total = torch.cat(features_total)

    features_vect.append(features_total)
...
# Saving features
torch.save(features_vect, f=r'E:\saved_model\features_test_' + model_type + '_' + img_type + '_epoch_' + str(epoch_done) +
                            '_' + dataset + '_' + str(time.gmtime()[2]) + str(time.gmtime()[1]) + '_' +
                            str(time.gmtime()[3]) + str(time.gmtime()[4]) + '.pkl')

torch.save(test_targets_vect, f=r'E:\saved_model\targets_test_' + model_type + '_' + img_type + '_epoch_' + str(epoch_done) +
                            '_' + dataset + '_' + str(time.gmtime()[2]) + str(time.gmtime()[1]) + '_' +
                            str(time.gmtime()[3]) + str(time.gmtime()[4]) + '.pkl')


# try:
#     test_df.to_excel(r'E:\saved_model\features_' + model_type + '_' + img_type + '_epoch_' + str(epoch_done) + '_' + dataset + '_' +
